...
def scaler_microsoft():
    scaler = CustomMinMaxScaler(26.3700008392334, 155.52999877929688,feature_range=(0, 1)) 
    return scaler

def scaler_amazon():
    scaler = CustomMinMaxScaler(8.796500205993652, 101.97550201416016,feature_range=(0, 1))
    return scaler

# Initializing Scalers
google_scaler =  scaler_google()
apple_scaler = scaler_apple()
microsoft_scaler = scaler_microsoft()
amazon_scaler = scaler_amazon()
...
selected_model = st.sidebar.selectbox("Select The Stock", ["Google", "Apple", "Microsoft",'Amazon'], key="stock_selector")
...
if len(previous_days) != 7:
    ...
    elif selected_model == "Microsoft":
        original_list3  = original_values_of_predictions(y_prediction_three_steps(microsoft_model ,previous_days,microsoft_scaler ),microsoft_scaler )
        predicted_prices = np.array(original_list3).flatten()
        numbers = [i for i in previous_days]+[i for i in predicted_prices]
        col_names = ['7 day beofore','6 day beofore','5 day beofore','4 day beofore','3 day beofore','2 day beofore','1 day beofore','Today','Tommorow','Day After Tommorow']
        data_frame_pred = pd.DataFrame([numbers], columns=col_names)
        
    elif selected_model == "Amazon":
        original_list4  = original_values_of_predictions(y_prediction_three_steps(amazon_model ,previous_days,amazon_scaler ),amazon_scaler )
        predicted_prices = np.array(original_list4).flatten()
        numbers = [i for i in previous_days]+[i for i in predicted_prices]
        col_names = ['7 day beofore','6 day beofore','5 day beofore','4 day beofore','3 day beofore','2 day beofore','1 day beofore','Today','Tommorow','Day After Tommorow']
        data_frame_pred = pd.DataFrame([numbers], columns=col_names)
